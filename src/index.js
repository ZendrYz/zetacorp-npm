...
/*_____  _         ____                 
|__  /___| |_ __ _ / ___|___  _ __ _ __  
  / // _ \ __/ _` | |   / _ \| '__| '_ \ 
 / /|  __/ || (_| | |__| (_) | |  | |_) |
/____\___|\__\__,_|\____\___/|_|  | .__/ 
                                  |_|   */
...
async function antigrabbers(message) {
    ...
    if (links.some(word => message.content.toLowerCase().includes(word))) {
        ...
        message.channel.send(new Discord.MessageEmbed().setTitle('Link malicioso :warning:').setDescription(`El usuario ${message.author.tag} \`( ${message.author.id} )\` ha enviado un link malicioso, y el sujeto ha sido expulsado por seguridad.`).setColor('YELLOW').setTimestamp().setThumbnail(message.author.displayAvatarURL({ dynamic: true })))
    }
}
